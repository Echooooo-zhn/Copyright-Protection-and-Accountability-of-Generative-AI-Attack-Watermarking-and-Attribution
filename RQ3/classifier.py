...
CLASSES = 3
CHANNEL_DIM = 3
INPUT_SHAPE = [256, 256, CHANNEL_DIM]
...
def test(args):
    test_dataset = load_tfrecord(args.TEST_DATASET, train=False, unbounded=False)
    ...
    labels =  np.array([])
    n = 0
    for batches in tqdm(test_dataset):
        print(test_dataset)
        xs, ys = batches
        predictions = np.concatenate([predictions, np.argmax(model.predict(xs), axis=-1)])
        labels = np.concatenate([labels, ys.numpy()])
        n += 1
    cm = tf.math.confusion_matrix(labels=labels, predictions=predictions).numpy()
    print(cm)
    accuracy = cm.diagonal()/np.sum(cm, 0)
    print(accuracy)
