...
class ImageTranslationStarGAN(Experiment):
    def __init__(self, config):
        print('IMAGE TRANSLATION EXPERIMENT\n')
        super(ImageTranslationStarGAN, self).__init__(config)
    ...
    def configure_dataset(self):
        ...
        # print(f'self.config.dataset: {self.config.dataset}')
        # print(f'self.config.dataset.name: {name}')
    ...
    def train(self, **kwargs):
        ...
        for _ in range(d_iter):
            data = {
                'real_sample': self.model.real_sample,
