...
class ResnetGeneratorAttentionV2(nn.Module):
    # initializers
    def __init__(self, input_nc=3, output_nc=3, ngf=64, n_blocks=9):
        super(ResnetGeneratorAttentionV2, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        self.nb = n_blocks
        self.conv1 = nn.Conv2d(input_nc, ngf, 7, 1, 0)
        self.conv1_norm = nn.InstanceNorm2d(ngf, affine=True)
        self.conv2 = nn.Conv2d(ngf, ngf * 2, 3, 2, 1)
        self.conv2_norm = nn.InstanceNorm2d(ngf * 2, affine=True)
        self.conv3 = nn.Conv2d(ngf * 2, ngf * 4, 3, 2, 1)
        self.conv3_norm = nn.InstanceNorm2d(ngf * 4, affine=True)
        
        
        # nn.init.normal_(self.conv1_norm.weight.data)
        # nn.init.constant_(self.conv1_norm.bias.data, 0.0)
        # nn.init.normal_(self.conv2_norm.weight.data)
        # nn.init.constant_(self.conv2_norm.bias.data, 0.0)
        # nn.init.normal_(self.conv3_norm.weight.data)
        # nn.init.constant_(self.conv3_norm.bias.data, 0.0)
        ...
        self.deconv1_norm_content = nn.InstanceNorm2d(ngf * 2, affine=True)
        self.deconv2_content = nn.ConvTranspose2d(ngf * 2, ngf, 3, 2, 1, 1)
        self.deconv2_norm_content = nn.InstanceNorm2d(ngf, affine=True)
        self.deconv3_content = nn.Conv2d(ngf, 27, 7, 1, 0)

        self.deconv1_attention = nn.ConvTranspose2d(ngf * 4, ngf * 2, 3, 2, 1, 1)
        self.deconv1_norm_attention = nn.InstanceNorm2d(ngf * 2, affine=True)
        self.deconv2_attention = nn.ConvTranspose2d(ngf * 2, ngf, 3, 2, 1, 1)
        self.deconv2_norm_attention = nn.InstanceNorm2d(ngf, affine=True)
    ...
    def forward(self, input):
        ...
        # x_content = F.relu(self.deconv1_content(x))
        # x_content = F.relu(self.deconv2_content(x_content))
        ...
        # x_attention = F.relu(self.deconv1_attention(x))
        # x_attention = F.relu(self.deconv2_attention(x_attention))
...
class resnet_block(nn.Module):
    def __init__(self, channel, kernel, stride, padding):
        ...
        self.conv1_norm = nn.InstanceNorm2d(channel, affine=True)
        self.conv2 = nn.Conv2d(channel, channel, kernel, stride, 0)
        self.conv2_norm = nn.InstanceNorm2d(channel, affine=True)
