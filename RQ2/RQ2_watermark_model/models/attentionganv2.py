...
class AttentionGANv2(Model):
    def __init__(self, config, device=[torch.device('cpu'), ]):
        super(AttentionGANv2, self).__init__()
        ...
        print(f'config.epoch: {config.epoch}')
        print(config)
    ...
    def forward_g(self, data):
        ...
        self.fake_B, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GA(self.real_A)
        
        self.fake_A, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GB(self.real_B)
        
        
        self.rec_A, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GB(self.fake_B)  # G_B(B)
        
        self.rec_B, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GA(self.fake_A)
        
        
        self.idt_A, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GA(self.real_B)
        
        self.idt_B, _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _, _, \
        _, _, _, _, _, _, _, _, _ = self.GB(self.real_A)
